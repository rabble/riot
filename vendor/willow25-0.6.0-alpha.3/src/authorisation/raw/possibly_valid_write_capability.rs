...
wrapper! {
    /// A write capability that has not been checked for [validity](https://willowprotocol.org/specs/meadowcap/index.html#cap_valid).
    ///
    ...
    PossiblyValidWriteCapability; meadowcap::raw::PossiblyValidWriteCapability<MCL, MCC, MPL, NamespaceId, NamespaceSignature, SubspaceId, SubspaceSignature>
}

impl fmt::Debug for PossiblyValidWriteCapability {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(f)
    }
}

impl PossiblyValidWriteCapability {
    ...
    pub fn new_communal(namespace_key: NamespaceId, user_key: SubspaceId) -> Self {
        Self(meadowcap::raw::PossiblyValidWriteCapability::new_communal(
            namespace_key,
            user_key,
        ))
    }
    ...
    pub fn new_owned(
        ...
        Self(meadowcap::raw::PossiblyValidWriteCapability::new_owned(
            namespace_key,
            user_key,
    ...
    /// Converts `self` into a [`WriteCapability`] **without enforcing [validity](https://willowprotocol.org/specs/meadowcap/index.html#cap_valid)**.
    ///
    ...
    pub unsafe fn into_write_capability_unchecked(self) -> WriteCapability {
        // SAFETY: the contract for safe calls to this function is identical to that of the function call in the unsafe block.
        unsafe { self.0.into_write_capability_unchecked().into() }
    }
    ...
    /// Converts `self` into a [`WriteCapability`], or returning `Err(self)` if `self` is not [valid](https://willowprotocol.org/specs/meadowcap/index.html#cap_valid).
    ///
    /// See [`into_write_capability_unchecked`](PossiblyValidWriteCapability::into_write_capability_unchecked) for the unsafe (but more efficient) version of this.
    #[allow(clippy::result_large_err)]
    pub fn into_write_capability(self) -> Result<WriteCapability, Self> {
        match self.0.into_write_capability() {
            Ok(yay) => Ok(yay.into()),
...
impl Encodable for PossiblyValidWriteCapability {
...
impl EncodableKnownLength for PossiblyValidWriteCapability {
    fn len_of_encoding(&self) -> usize {
        self.0.len_of_encoding()
    }
}

/// Implements decoding according to the [EncodeMcCapability](https://willowprotocol.org/specs/encodings/index.html#EncodeMcCapability) encoding relation, and further errors if the decoded capability is a write capability. **Does not check the validity** of the decoded capability.
impl Decodable for PossiblyValidWriteCapability {
    ...
    where
        ...
        Self: Sized,
    {
        let decoded: meadowcap::raw::PossiblyValidWriteCapability<
            MCL,
            MCC,
            MPL,
            NamespaceId,
...
/// Implements decoding according to the [encode_mc_capability](https://willowprotocol.org/specs/encodings/index.html#encode_mc_capability) encoding function, and further errors if the decoded capability is a write capability. **Does not check the validity** of the decoded capability.
impl DecodableCanonic for PossiblyValidWriteCapability {
...
impl From<WriteCapability> for PossiblyValidWriteCapability {
    fn from(value: WriteCapability) -> Self {
        meadowcap::raw::PossiblyValidWriteCapability::<
            MCL,
            MCC,
            MPL,
            NamespaceId,
            ...
            SubspaceId,
            SubspaceSignature,
        >::from(meadowcap::WriteCapability::<
            MCL,
            MCC,
            MPL,
            NamespaceId,
