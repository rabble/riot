...
impl Component {
    /// Creates a reference to a [`Component`] from a reference to a byte slice. Returns [`None`] if the slice is longer than 4096 ([`MCL`]).
    ///
    ...
    pub fn new(s: &[u8]) -> Result<&Self, InvalidComponentError> {
        Ok(wdm::Component::<MCL>::new(s)?.into())
    }

    /// Creates a reference to a [`Component`] from a reference to a byte slice, without ensuring it consists of no more than 4096 ([`MCL`]) bytes.
    ///
    ...
    pub unsafe fn new_unchecked(s: &[u8]) -> &Self {
        unsafe { wdm::Component::<MCL>::new_unchecked(s).into() }
    }

    /// Creates a mutable reference to a [`Component`] from a mutable reference to a byte slice. Returns [`None`] if the slice is longer than 4096 ([`MCL`]).
    ///
    /// Aside from checking the length, this is a cost-free conversion.
    ///
    /// #### Examples
    ...
    pub fn new_mut(s: &mut [u8]) -> Result<&mut Self, InvalidComponentError> {
        Ok(wdm::Component::<MCL>::new_mut(s)?.into())
    }

    /// Creates a mutable reference to a [`Component`] from a mutable reference to a byte slice, without ensuring it consists of no more than 4096 ([`MCL`]) bytes.
    ///
    /// This is a cost-free conversion.
    ///
    /// #### Safety
    ...
    pub unsafe fn new_mut_unchecked(s: &mut [u8]) -> &mut Self {
        unsafe { wdm::Component::<MCL>::new_mut_unchecked(s).into() }
    }
    ...
    pub fn new_empty() -> &'static Self {
        wdm::Component::<MCL>::new_empty().into()
    }
...
impl IndexMut<usize> for Component {
    fn index_mut(&mut self, index: usize) -> &mut Self::Output {
        &mut self.0[index]
    }
}

impl Index<usize> for Component {
    type Output = u8;
...
impl Index<Range<usize>> for Component {
...
impl Index<RangeFrom<usize>> for Component {
    type Output = Component;
...
impl Index<RangeTo<usize>> for Component {
    type Output = Component;
...
impl Index<RangeFull> for Component {
    type Output = Component;
...
impl Index<RangeInclusive<usize>> for Component {
    type Output = Component;
...
impl Index<RangeToInclusive<usize>> for Component {
    type Output = Component;
...
impl AsRef<[u8]> for OwnedComponent {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}
...
impl<'a> From<&'a wdm::OwnedComponent<MCL>> for &'a OwnedComponent {
    fn from(value: &'a wdm::OwnedComponent<MCL>) -> Self {
        // Safe because `OwnedComponent` is `#[rep(transparent)]`.
        unsafe { &*(value as *const wdm::OwnedComponent<MCL> as *const Self) }
    }
}

impl<'a> From<&'a mut wdm::OwnedComponent<MCL>> for &'a mut OwnedComponent {
    fn from(value: &'a mut wdm::OwnedComponent<MCL>) -> Self {
        // Safe because `OwnedComponent` is `#[rep(transparent)]`.
        unsafe { &mut *(value as *mut wdm::OwnedComponent<MCL> as *mut Self) }
    }
}
...
impl<'a> From<&'a OwnedComponent> for &'a wdm::OwnedComponent<MCL> {
    fn from(value: &'a OwnedComponent) -> Self {
        // Safe because `OwnedComponent` is `#[rep(transparent)]`.
        unsafe { &*(value as *const OwnedComponent as *const Self) }
    }
}

impl<'a> From<&'a mut OwnedComponent> for &'a mut wdm::OwnedComponent<MCL> {
    fn from(value: &'a mut OwnedComponent) -> Self {
        // Safe because `OwnedComponent` is `#[rep(transparent)]`.
        unsafe { &mut *(value as *mut OwnedComponent as *mut Self) }
    }
}
...
impl Index<usize> for OwnedComponent {
    type Output = u8;
...
impl Index<Range<usize>> for OwnedComponent {
    type Output = Component;
...
impl Index<RangeFrom<usize>> for OwnedComponent {
    type Output = Component;
...
impl Index<RangeTo<usize>> for OwnedComponent {
    type Output = Component;
...
impl Index<RangeFull> for OwnedComponent {
    type Output = Component;
...
impl Index<RangeInclusive<usize>> for OwnedComponent {
    type Output = Component;
...
impl Index<RangeToInclusive<usize>> for OwnedComponent {
    type Output = Component;
...
mod tests {
    ...
    fn index_mut() {
        ...
        let compo = Component::new_mut(base).unwrap();
    ...
    fn index() {
        ...
        let compo = Component::new(b"sunshine").unwrap();
    ...
    fn owned_index() {
        let slice = &b"sunshine"[..];
        let compo = OwnedComponent::new(b"sunshine").unwrap();
        assert_eq!(slice[1], compo[1]);
        assert_eq!(slice[5], compo[5]);
    }
