...
- [`Deln0r/willow-go`](https://github.com/Deln0r/willow-go), including corrected-WILLIAM3 commit [`9d848ee`](https://github.com/Deln0r/willow-go/commit/9d848ee), only as an independent interoperability cross-check, never as protocol authority.
...
7. **An accepted entry can be dominated immediately.** Planning/receipt outcomes therefore need `WouldApply`/`AppliedAtCommit`, `WouldBeDominated`/`DominatedAtCommit`, and `AlreadyPresent`, not only `Inserted` and `AlreadyPresent`.
...
11. **Multi-entry import must be one batch join, not sequential receipt semantics.** Willow's live result is order-independent. Riot must join the pre-state with the complete selected namespace batch, then derive dispositions from the pre-state and final state; otherwise receipts can depend on bundle order even when the live view does not.
...
Riot separates canonical Willow-value identity from the proof carried by a bundle:

```text
entry_id = SHA-256(
  "riot/willow-entry-id/v1" ||
  u32be(len(entry_bytes)) || entry_bytes
)

evidence_digest = SHA-256(
  "riot/evidence-digest/v1" ||
  u32be(len(entry_bytes)) || entry_bytes ||
...
Phase 0A accepts only communal namespaces with a zero-delegation communal capability for the entry subspace. Generic Meadowcap-valid owned or delegated proofs remain unsupported, so a proof variant cannot silently change Phase 0A deduplication semantics.

### Time and path

One session-owned fallible `ClockSource` read provides a `ClockSnapshot`:

- `willow_timestamp`: `willow25::Timestamp`, microseconds TAI since J2000, used for join recency;
- `created_at`: UTC Unix seconds inside the signed alert payload, from the same instant as the Willow timestamp;
- user-authored `valid_from` and `expires_at`: UTC Unix seconds validated relative to that snapshot;
...
├── seen_entry_index: entry_id -> first receipt facts
└── receipts
```
...
- every batch permutation produces the same dispositions keyed by canonical entry ID; only receipt row presentation order may follow bundle order.

### Import dispositions

Per selected entry, after computing one final namespace join for the whole selected batch:
...
Stable `entry_id` hashes only canonical Entry bytes, avoiding proof- and order-dependent allocation. `evidence_digest` separately binds capability and signature. `AppliedAtCommit` means the entry is in that commit's final live namespace view; its pruned list names only directly dominated entries removed from the pre-commit live view, not same-batch candidates. `DominatedAtCommit` means it was valid and accepted into local receipt/index history but is absent from the final view; its dominators come from that final view. `AlreadyPresent` is a replay of an entry ID already accepted locally.

A commit containing a previously unseen dominated entry changes the seen index and creates a receipt, so it increments the Riot store generation. A duplicate-only commit remains `NoChanges` and creates no second receipt. Historical provenance retains the immutable receipt disposition separately from current `Live` or `NotLive { DominatedOnArrival | PrunedLater }` status.
...
Decode order is bounded canonical outer CBOR, canonical Entry, canonical capability, fixed signature, payload length/digest, checked Meadowcap authorisation, Riot schema, then preview policy. No untrusted entry reaches join staging before all prior checks pass. Outer-frame/cumulative failures reject the artifact; once a bounded canonical item frame is isolated, component failures become structured ineligible-item diagnostics so a bad sibling cannot hide good entries.
...
The existing WU0 environment and native-platform preflight remain valid. Although commit `50083bb` updated the resolved pins, review keeps the dependency portion **REVISE** until all of the following are executable and independently checkable:

1. update the workspace pin and fixture manifest;
2. regenerate `Cargo.lock` and its recorded SHA-256;
3. add corrected WILLIAM3 empty, short, partial-block, and multi-block golden vectors with input recipes, version, hashes, and at least one expected value cross-checked against an independent corrected implementation;
4. make `cargo xtask validate-contracts` reject the old version, wrong lock hash, missing vector hash, Drop Format, and an aborting evidence profile;
5. use `panic = "unwind"` for the Phase 0A FFI release artifacts so the required catch/quarantine behavior can be tested;
6. rerun the five-target compile probe and `cargo tree -e features`;
7. update the WU0 report and aggregate-agent-hour ledger with the evidence actually obtained.
