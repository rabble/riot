...
Date: 2026-07-10 (last status update: 2026-07-14)
Status: **Phase 0A COMPLETE.** G0, G1, and full G2 all PASS. All work units WU0–WU3 (including Tasks 4, 5, and 5b) are merged to `main`. The earlier "open sequencing question" about WU3 starting before formal full-G2 — flagged in prior revisions of this spec — is resolved: full G2 evidence was subsequently proven and WU3 shipped. The platform pivot (shared Rust core, native iOS + Android shells) is confirmed and delivered. The gateway demo track also landed and now carries real per-entry signature verification. Follow-on work (newswire core, SQLite persistence layer, sync wire protocol) has since been merged as the foundation for Phase 1.
