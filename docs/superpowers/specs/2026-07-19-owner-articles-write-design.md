...
**Schema (additive, backward-compatible):** add `sections: Vec<Vec<u8>>` to `SiteManifestV1` (each entry a section-name component, bounded like `MAX_SECTION_BYTES = 64`, list bounded e.g. ≤ 64 sections) as **new CBOR key 6**; `decode_site_manifest` treats an absent key 6 as `sections = []` (existing signed manifests still decode unchanged — the numbered-key codec + `prove_canonical` already support this). `validate_structure` bounds the list; `SiteLayout` is untouched. The signature covers the new encoding; nothing about the existing `version`/floor/`require`-monotonicity mechanics changes.
...
- `OwnedMasthead::open_sealed` → build `SiteManifestV1{ root = owner key, members, moderation_path, transport_policy, sections, version }` → `encode_site_manifest` → `authorise_owner_entry` at `[MANIFEST_COMPONENT]` (owner-only, same masthead-secret guarantee as `/mod` and `/articles`) → import through the **durable version-floor gate** (`admit_manifest_version`): a strictly-higher `version` is accepted; a lower one is a rollback refusal; same `version` different content is an equivocation refusal (existing `version_floor` mechanics — the durable floor DB is required, so this path is durable-only). Zeroize the key.
- **`publish_site_manifest` MUST NOT touch `sync_inventory`** (same cross-community isolation as `import_owned_article`), tested.
- Declaring/adding a section = re-publish the manifest with the section appended + `version + 1`. There is no separate "section record" — sections are manifest state (site structure belongs in the signed manifest, consistent with `members`/`layout`; contrast `Endorse`, a per-record moderation *action* deliberately kept out of the manifest).
...
- **The FULL degradation is shared, not just freshness (Security-strengthened).** `article_feed_render` needs the complete `CompositeDegradation` — `resolve_degradation` folds SIX inputs (`manifest_valid`, `all_members_verified`, `floor`, `moderation`, `transport_blocked`, `comments_and_wire_synced`). Deriving the article verdict from `ModerationFreshness` alone would silently omit the **identity-in-question** holds (`ManifestInvalid`/`EquivocationAlarm`/`ManifestRollbackAlarm`/`TransportBlocked`) — i.e. a compromised/rolled-back masthead would render editorial articles as clean, the exact top-threat leak. So: extract the WHOLE degradation computation currently inlined in `resolve_composite_site_from_store` (the held/protected scan **and** the `resolve_degradation(&DegradationInputs{…})` call) into one shared helper `fn resolve_site_degradation(store, root, now) -> CompositeDegradation`; **both** `resolve_composite_site` and `resolve_article_feed_from_store` consume the identical value — structurally incapable of diverging. Unit 3 characterize-locks `resolve_composite_site`'s prior behavior first, then adds a **cross-surface consistency test**: article-hold and trust-surface-hold agree for every one of the 8 variants. (Note: `floor`/`transport_blocked` are currently hardcoded-benign stubs in `resolve_composite_site_from_store` — so Equivocation/Rollback/TransportBlocked cannot fire yet; sharing the computation means the article path lights up automatically when those stubs are wired, never left behind.)
...
- **Sections are manifest-declared + owner-signed.** An article's `section` must be in the current signed manifest's `sections`; the check is one shared core validator (`section_is_declared`) used by both write and resolve. Publishing/updating the manifest (incl. sections) is owner-only (masthead secret), version-floor-gated (higher version accepted, rollback/equivocation refused), and never mutates `sync_inventory`. A manifest is backward-compatible: an old manifest with no `sections` key decodes as none declared.
...
- **Manifest/sections (Unit 0):** existing signed manifests still decode (absent `sections` key → `[]`); a re-published manifest with a higher `version` + new section admits (floor), a lower version is a rollback refusal, same-version-different-content an equivocation refusal; `publish_site_manifest` is owner-only and leaves `sync_inventory` unchanged; `section_is_declared` accepts declared, rejects undeclared.
