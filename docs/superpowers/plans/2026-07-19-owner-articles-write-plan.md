...
- Branch `feat/composite-owner-articles-write` (off `origin/main`). Green baseline: **`cargo test -p riot-core -p riot-ffi --all-features`** (the `--all-features` is REQUIRED — several tests, e.g. `identity_namespace_shape`, use `#[cfg(feature = "conformance")]` exports like `generate_communal_author_with`/`OsEntropy`; omitting the flag makes them look "broken"). NOTE: this branch is ~14 commits behind `origin/main` (incl. #73 M1 protocol core, #76) — rebase onto current `origin/main` at PR time; the site-module feature work is orthogonal to those.
