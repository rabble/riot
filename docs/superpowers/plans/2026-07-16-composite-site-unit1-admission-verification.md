...
| 4a | `crates/riot-core/src/newswire/entry.rs:326` (`if capability.is_owned()`) | **separate 4th gate** — newswire inspection/projection; editorial articles hit this | **Must be updated consistently** (genuine second policy site, not a routing seam). |
...
- **Task 6 — retire the string-roster.** The app-level `editorial_roster` (`newswire_ffi.rs:43`) is replaced/backed by the cryptographic cap check, not a name-compare. RED: an author NOT in a legacy string roster but holding a valid delegated cap is admitted; an author in a string roster but WITHOUT a valid cap is rejected. Coordinate with any FFI consumers of `editorial_roster` (native call sites) — if the field must stay for compat, make it display-only and prove the cap check is authoritative.
