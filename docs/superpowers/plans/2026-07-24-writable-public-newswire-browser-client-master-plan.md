...
Codebase verified 2026-07-27 against branch `feat/identity-handles-logging-share` at merge commit `d0be6e7a` (base branch + merged `fix/riverside-member-tool-uitest`: sqlite fix rusqlite 0.40.1/bundled 3.53.2, vendor precedent, riverside features) — the execution base declared in §Execution isolation. All file:line citations below are from that tree. These findings override assumptions; unit plans must respect them.
...
2. **willow25 is NOT vendored.** It comes from crates.io (`Cargo.lock:8096`), with non-optional `async-fs 2.2.0` and `fjall 3.0.3` deps; `storage/mod.rs:16-24` gates `persistent_store` only on `std`. The spec's vendor patch (new `persistent-storage` feature gating `persistent_store` + `PersistentStore` re-export behind `all(feature = "std", feature = "persistent-storage")`) matches source reality and is WU-000. **Gate-discovered extension:** `async-fs` also enters via `bab_rs`'s default `storage` feature (`bab_rs 0.8.1`: `storage = ["std", "dep:async-fs", "dep:futures-lite"]`), and willow25's unconditional modules (`storage/store.rs`, `payload_prefix_store.rs`, `memory_store.rs`) require `bab_rs::generic::storage`, so willow25 cannot compile without bab_rs `storage`. WU-000 therefore also vendors `bab_rs 0.8.1` with a second minimal patch: `async-fs`/`futures-lite` optional behind a new `storage-fs` feature gating only `generic/storage.rs`'s `backend_filesystem` module (the sole async_fs/futures_lite user). Note willow25's `persistent_store.rs:25-31` builds on `backend_filesystem::{FileBackend, KeyState}`, so willow25's `persistent-storage` feature must additionally enable `bab_rs/storage-fs` — native keeps the full chain, the Wasm shape (no `persistent-storage`) drops it. Riot's direct bab_rs use is already `default-features = false, features = ["william3"]` (root `Cargo.toml:21`); nothing in `crates/` touches `backend_filesystem` directly. Vendor-patch precedent exists on the execution base: root `Cargo.toml:66` `[patch.crates-io]`, `:13` workspace `exclude`, and `vendor/tor-dirmgr-0.44.0/RIOT_PATCH.md` (convention: verbatim upstream source, provenance + license + integrity notes, single-justification patch). WU-000 follows that convention.
...
4. **Whole-file import rejection is riot-client's job.** `decode_bundle` (`crates/riot-core/src/import/bundle.rs:276`) isolates per-item (`ItemStatus::Invalid`) and `inspect` declines to carry ineligible items into the preview (`session.rs:854`, "Ineligible items are simply not carried into the preview"). Spec §Import-2 demands atomic whole-file rejection before selection; riot-client gates on `DecodedBundle.items` itself.
...
6. **`MemoryEvidenceStore` is `pub(crate)`** (`store/memory.rs:7`); riot-client reaches memory behavior only via `RiotSession::open()` (`session.rs:381`). The capability/signature drop after admission lives in the private `Stored` struct (`import/join.rs:55`, documented at `join.rs:47-64`) — this is why riot-client owns a separate exact-proof ledger for export.
7. **`commit_at` is `pub(crate)`** (`session.rs:933`). riot-client posts via the public sequence: `create_signed_*` → `encode_bundle` → `inspect` → `preview.plan(...)` → `plan.commit()`.
...
Each work unit executes in an isolated worktree created via the `superpowers:using-git-worktrees` skill at execution time, on a branch named `feat/browser-client-wuNNN-<slug>`. **Base branch is `feat/identity-handles-logging-share` at merge commit `d0be6e7a` or later** (spec commit `8521a324` plus the merged sqlite-fix/vendor-precedent tree), not `main`. Commits use explicit `git add <paths>` (never `git add -A`/`git add .`), pathspec-limited to the unit's file ownership. Conventional-commit messages. Never `--no-verify`. TDD is mandatory: every behavior lands test-first, observed failing.
...
|---|---|
| WU-000 | `vendor/willow25-0.6.0-alpha.3/**`, `vendor/bab_rs-0.8.1/**`, root `Cargo.toml` `[patch.crates-io]` + `exclude`, `crates/riot-core/Cargo.toml` (feature passthrough only), `Cargo.lock`, `scripts/web/check-wasm-graph.mjs` (created here, extended in WU-003) |
...
- Create: `vendor/bab_rs-0.8.1/**` (verbatim crates.io source), `vendor/bab_rs-0.8.1/RIOT_PATCH.md`
- Modify: `vendor/willow25-0.6.0-alpha.3/Cargo.toml` (fjall/async-fs optional behind `persistent-storage`, which also enables `bab_rs/storage-fs`; bab_rs dep pinned `default-features = false, features = ["william3", "storage"]`), `vendor/willow25-0.6.0-alpha.3/src/storage/mod.rs` (gate module + re-export)
- Modify: `vendor/bab_rs-0.8.1/Cargo.toml` (async-fs/futures-lite optional behind `storage-fs`), `vendor/bab_rs-0.8.1/src/generic/storage.rs` (gate `backend_filesystem`)
- Modify: root `Cargo.toml` (`[patch.crates-io]` willow25 + bab_rs, workspace `exclude`), `Cargo.lock`
- Create: `scripts/web/check-wasm-graph.mjs` (graph contract assertion; extended in WU-003)

- [ ] Vendor both upstream sources; verify each `.crate` archive checksum against `Cargo.lock` (willow25 `6477a05f…` near `:8096`, bab_rs `b80f2b89…`); record provenance, license retention, and per-file integrity manifests in each `RIOT_PATCH.md`
...
- [ ] Patch: bab_rs `storage-fs` feature + `backend_filesystem` gate; willow25 `persistent-storage` feature; gate `persistent_store` module + `PersistentStore` re-export on `all(feature = "std", feature = "persistent-storage")`; riot-core's native/default consumers enable it, Wasm graph disables it
- [ ] GREEN: graph contract passes; `cargo test --workspace --all-features` green (native behavior unchanged)
- [ ] `xtask validate-contracts` updated if it pins willow25/bab_rs metadata; `fixtures/manifest.json` `cargo_lock_sha256` refreshed
- [ ] Commit
