...
**Equal-timestamp correction (human decision: “do it the Willow way”):** Trust and Revoke at the same namespace/subspace/path remain one ordinary Willow coordinate. When their entry timestamps tie, Willow's native payload-digest recency selects exactly one live entry, independent of import order. Riot does not impose Revoke precedence inside that coordinate. The Revoke-wins tie in `is_trusted` is a separate reader policy across different organizer coordinates after Willow has selected each coordinate's live entry.
...
> **Execution-time security correction (2026-07-11):** A manifest alone cannot prove the content-derived app ID in its path. Putting its metadata into `IndexedApp` therefore lets a carrier choose another app's identity and influence trust or supersession before the bundle arrives. `scan_app_index` must instead return decoded manifest-only records in a separate, explicitly unverified `pending_manifests` collection containing the claimed path ID and full carrier coordinate/timestamp. Pending records may drive a “Still arriving” presentation, but must never enter `DirectoryInputs.apps`, trust, supersession, or launch decisions. Only a strict manifest+bundle pair at the same namespace/subspace coordinate, with matching entry points and a recomputed app ID equal to the path, is promoted to `IndexedApp`; bundle-only records remain absent. Endorsements are globally reconciled by `(app_id, endorser_subspace_id)` across namespace copies before the 256-per-app output cap: newest timestamp wins, equal-time disagreement fails closed to retracted, and remaining ties use immutable Willow identity bytes. The ceiling bounds materialized output, not scan allocation or protocol validity.
