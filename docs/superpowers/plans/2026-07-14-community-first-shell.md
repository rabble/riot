...
12. **⚠️ FLAGGED DESIGN DECISION (Unit 3) — per-community sealed identity. OWNER RATIFICATION, like CONFIRM #2.** Faithful multi-community requires holding *organizer-of-A* and *member-of-B* **simultaneously with identity continuity**, but today `LocalProfile` holds ONE `author` and `join_public_space` **destructively regenerates** it (both key-gen paths mint fresh random keypairs, non-re-derivable). The approved specs only gesture at this ("the real immutable SpaceSession"). **Decision made in the owner's absence (2026-07-15):** each community's author keypair is **sealed and stored per registry row via the EXISTING `seal_identity` mechanism** (re-sealed with the profile wrapping key) — no new crypto, no new key-derivation. Switch is the only path that unseals a community's author, and it re-seals the outgoing one. A corrupt sealed row **quarantines** (reuses `authority_quarantined`), never drops or leaks a partial key. **Why proceed rather than wait:** it reuses a reviewed mechanism, it is the only faithful path to multi-community, and the coordinator runs an **independent adversarial isolation review** targeting the at-rest sealing (a sealed B author must be genuinely un-loadable while A is active). **Owner to ratify** the per-community-sealed-identity model, or direct a different one. Do not describe multi-community identity as final until ratified.
