...
| **3 multiple communities (registry; SECURITY)** | ✅ done + ratified | 3A `1e29f06`+`71804d3`, 3B `6fa0c10`+`80f3a5f`, 3C `2d4f2bc` | **The final unit.** SQLite community registry, per-community DISTINCT sealed identity (unlinkable; Case-1 durable on both platforms), isolated switch, fail-closed switch/write race via the (now genuinely wired) community-generation guard, quarantine on bad key / corrupt blob, archive/restore. **Security core TWICE independently audited: isolation HOLDS + phantom-guard fix COMPLETE-AND-COVERED.** iOS 297/295 (2 known-red), CommunityChooserTests 12/12 incl real `<300ms` hard-gate switch, ShellNav 28/28, both apps build; Android 136/0; Rust persistence_contract 13/13, clippy 0. **Risk 11 CLOSED** — newswire Home reprojects on reopen. **Owner-ratify: per-community sealed identity (Risk 12).** **Residual: unsealed-author RAM window between join and persist (Risk 13).** **Scoped out: multi-community JOIN affordance in the app UI** (create-N + switch works; joining a SECOND community while holding one is single-community-gated at the app wrapper — the FFI supports it; DoD #1 "follow multiple" — estimate pending for 3D-vs-residual). |

**PLAN COMPLETE (2026-07-15):** all units P1 → 3 landed and independently re-verified against a fresh native build. Combined final state: Rust workspace green (persistence_contract 13/13, clippy `-D warnings` 0, fmt clean); **iOS full RiotKit 297 tests / 295 pass — only the 2 pre-existing known-red `TwoPeerNearbySyncTests` Bonjour failures**; iOS Riot.app + macOS Riot-macOS both BUILD SUCCEEDED; Android `testDebugUnitTest` 136 / 0. Two security gates found + fixed REAL issues (2B nearby disclosure leak; 3A phantom generation-guard), each re-verified by an independent adversarial auditor. **Not done / owner decisions:** web/gateway half (deferred, CONFIRM #1), MLS/private groups (deferred by owner), the whole-product human trial (§8.3, needs evaluators), and the ratification/residual items in §8.5 (Risks 6/2, 9, 10, 12, 13) + the join-second-community UI. Two-peer physical BLE/Bonjour sync remains **known-red and unclaimed**.
...
9. **⚠️ SECURITY RESIDUAL (0C `d9699e8`; content-scan added `sec/residuals`) — WebRTC egress is now refused-at-import, not merely best-effort-disabled at runtime.** The hosted-app egress backstop (`WKContentRuleList`, fail-closed) blocks every URL-loader vector, proven with CSP stripped: fetch/XHR/WebSocket/EventSource/beacon/img/script/link/iframe/form/css-url/dns-prefetch/preconnect/favicon → zero connections. But **WebRTC (`RTCPeerConnection` → STUN/TURN) does not flow through the URL loader**, so a content rule list cannot block it, and the runtime WKWebView lever is only a **private** `peerConnectionEnabled` preference (best-effort, App-Store-review risk, can break on an OS update). **Now closed at the earlier gate:** the shared Rust core refuses to host a bundle whose bytes reference any WebRTC API — `bundle::scan_bundle_egress` scans the **raw bytes** of every resource and returns `AppsError::BundleUsesWebRtc`, enforced at the single `verify_app_pair` chokepoint so **install, publish, scan, and share all refuse it identically on iOS and Android**. **Precise on which token closes what:** `RTCPeerConnection` / `webkitRTCPeerConnection` / `mozRTCPeerConnection` / `RTCDataChannel` are the **EGRESS** vector Risk 9 names (peer connection → STUN/TURN, invisible to the URL loader); `getUserMedia` / `navigator.mediaDevices` are **bonus camera/mic CAPTURE blocking**, not egress — blocking all six is broader-is-better, but the egress claim rests on the RTC* tokens. **Proven vs. assumed:** the refusal is *by content, not filename* (a WebRTC call hidden in an `image/png`-typed, `.png`-named resource is still refused — tested). **Honest residuals, both directions:** the scan is a literal substring match, so it is (a) **DENY-CLOSED** — it refuses a bundle that merely *mentions* a token (a comment, dead code, a vendored polyfill that names but never calls the API); for a tool that should host no WebRTC-capable code that is the intended tradeoff, documented in the error; and (b) **evadable** — an obfuscated or dynamically-assembled identifier (`window['RTC'+'PeerConnection']`) passes. It raises the bar; it is not a guarantee. The `WKContentRuleList` backstop and the disabled WebRTC preference remain the runtime net (defense in depth, both kept). Report app-runtime egress as **contained against URL-loader vectors and refused-at-import for statically-named WebRTC, deny-closed on mentions, with dynamic/obfuscated WebRTC an accepted residual** — not "fully contained."
...
**Isolation audit result (3A, `1e29f06`):** independent auditor verdict **HOLDS** — no exploitable cross-community leak or write-bypass; entries, trust markers, and coordinator handles are all namespace-scoped and cleared on switch; the race test proves store-absence in BOTH communities; authors are unlinkable (fresh OS entropy per join); wrong-key → quarantine, corrupt blob → quarantine, wrapping key zeroized on every path. Two overstated guards were found and are being hardened before ratification (not exploitable): (i) `community_generation` was write-only — the documented "captured + revalidated" guard was inert; being wired into the handle structs to match the real `app_execution_generation` pattern; (ii) the "at most the active author is ever unsealed" claim is true on disk but false in RAM (see Risk 13).
13. **✅ CLOSED (`sec/residuals`) — seal-inline-on-join.** Formerly: `join_public_space` parked the outgoing community's author **unsealed** in RAM until `persist_communities` sealed it — a running/unlocked-device RAM-forensics window. **Now closed:** `join_public_space` takes the wrapping key and seals the displaced author **INLINE** into its registry row via the existing `seal_identity` path (shared `seal_or_park_outgoing` helper, used by both join and switch), zeroizing the key on every path. With a key present, **only the ACTIVE author is ever unsealed in RAM** — proven by a unit test asserting `community_authors` holds no unsealed author after a keyed join, and by an FFI integration test that reopens a fresh handle (dropping all in-RAM parked authors) and recovers the joined-away community *without* an intervening `persist_communities`, which only succeeds because the join sealed it durably. **Keyless fallback retained:** an ephemeral `open_local_profile()` profile carries no key and still parks unsealed (nothing to seal with); real iOS/Android devices always pass the secure-store key (`withWrappingKey` / Keychain·Keystore). A seal failure (entropy) also falls back to parking so an author is never lost. The "only the active author is ever unsealed" claim now holds in RAM (with a key), not just on disk.
14. **SCOPED-OUT FEATURE (Unit 3) — nearby-adopt a SECOND community while holding one (DoD #1 "follow multiple", nearby half).** Unit 3 delivers the registry, chooser, switch, and isolation; **3D adds MANUAL multi-community join** (share-link / QR / paste → `join_public_space` when a community is already held). The remaining half — **adopting a second community over the nearby/`SpacePairing` flow** — is deliberately deferred because it would route 2B's *independently security-ratified* nearby-adopt path (single coordinator, adopt-replaces, bilateral-confirm gate) into a hold-and-switch multi-community model, **reopening that security surface**. Doing it safely needs another 2B-style adversarial disclosure/ownership pass. **Owner decision:** schedule the nearby-multi-adopt path as its own unit with a fresh security review, or accept manual join (3D) as sufficient for the MVP. Until then, a second community is joinable by share-reference but not by walking up to a peer.
15. **🔧 IN FLIGHT (`feat/join-descriptor`) — joined communities were a "dead follow"; descriptor-carry fix.** The shipped 3D `joinAdditionalCommunity` → `join_public_space` registered `descriptor_entry_id = None`, dropping the decoded 1E share reference's descriptor id. So a *joined* community's Home could never reproject even after a sync delivered the descriptor + posts — Risk 11 was closed only for CREATED communities. **Fix:** additive `join_newswire_community(space, descriptor_entry_id, wrapping_key)` — a thin wrapper over the SAME keyed multi-community `join_public_space` body that threads the descriptor through to `register_active_community` (already `Option<[u8;32]>`); `join_public_space` + its call sites UNCHANGED (2B/nearby-adopt untouched, no Android positional break). Proven RED→GREEN (`bef0c48`, persistence_contract 15/15: the joined row carries the handle, relationship Member). NECESSARY BUT NOT SUFFICIENT — see Risk 16.
16. **🔴 IN FLIGHT (`feat/join-descriptor`) — newswire content could not sync over the nearby bridge at all.** `import_signed_newswire` (create_newswire_space / create_newswire_post) commits via `plan_all().commit()` but NEVER called `install_sync_inventory`, unlike the alert (`sign_draft`) and app-data (`accept_plan`) commit paths. So a newswire profile's `sync_inventory ≠` its live entries → `ensure_complete_sync_inventory` fails → `open_sync_session` returns `Internal`. **Consequence:** a followed community's Home stays EMPTY FOREVER over nearby — the only delivery path today (gateway/web is the deferred half). This is what made "community + PUBLISHING" single-device. **Fix (`4bc4d09`):** `import_signed_newswire` adds its committed entry to `sync_inventory` — a newswire entry IS a live entry, so it belongs there and the `inventory == active-namespace-live-ids` invariant (load-bearing for 2B/3A cross-community isolation) still holds, namespace-scoped. **Required proof (in flight):** the full isolation + persistence suite stays green (no cross-community inventory leak), AND the end-to-end — create A → publish post → follower joins by share-ref → `open_sync_session` SUCCEEDS → sync delivers descriptor + post → follower's Home reprojects the post. Risks 15 + 16 + the recovered iOS 3D ship as one PR = "follow multiple **and see published content**." Coordinator runs an independent adversarial isolation pass on the inventory change before ratifying.
