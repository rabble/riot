...
**Modify:**
- `crates/xtask/src/main.rs` — **`mod` declarations land WITH the task that creates each
  module** (avoids the earlier double-add and the "verify tests can't compile" gap): Task 1.9
  adds `mod export_newswire;`; Task 2.3 adds `mod verify_newswire_export;` (its own `--lib`
  tests in 2.4/2.6 need it, so it CANNOT wait for Task 3). Task 3 then adds ONLY the two
  dispatch arms (alongside `main.rs:201-214`), both entries in `available_commands()`
  (`main.rs:228-235`), and extends the `conference_fixture_commands_report_success_and_failure`
  test (`main.rs:938-988`) — Task 3 does NOT touch `mod` lines. The `fn sha256_hex` →
  `pub(crate)` visibility bump rides with Task 1.9.
...
the conference modules' test density: pure helpers unit-tested with hand-built inputs; `run()`
tested against a temp-root copy of the committed fixtures; a missing-fixture failure arm; **AND
both new BRANCHES exercised — the `signature_invalid` verify arm (Task 2.6b) and the
moderation-drop `continue` arm (Task 1.4b)** — otherwise the llvm branch floor (83) regresses,
which the local composite gate (`scripts/web/coverage.sh`) blocks.
...
pub fn build_signed_newswire() -> Result<BuiltNewswire, String> {
    ...
    // path and Hidden content is proven to never reach the public export.
    records.push(
        create_signed_editorial_action(&editor, &descriptor_verified, action(post_ids[6], EditorialActionKind::Hide))
...
```rust
    ...
    fn run_writes_both_golden_fixtures_in_a_consistent_state() {
        ...
        assert_eq!(signed_records.len(), 1 + 7 + 6, "descriptor + 7 posts + 6 actions (2 Feature + 3 Verify + 1 Hide)");
...
First add the module declaration so its `--lib` tests compile — in `main.rs`, right after the
`mod export_newswire;` line added in Task 1.9, add:

```rust
mod verify_newswire_export;
```

Then create `crates/xtask/src/verify_newswire_export.rs`:
...
**2.6b — Add the invalid-signature branch test (exercises the `signature_invalid` stamp arm).**

The conference module has `tampered_entry_bytes_do_not_verify`; mirror it so the newswire's
`else { VERIFICATION_STATUS_INVALID }` arm is covered (this is what the removed integrity pass
would have made unreachable). Append to the `verify_newswire_export` tests:
...
```rust
    ...
    fn run_stamps_signature_invalid_when_a_bound_records_entry_bytes_are_tampered() {
        ...
        let target_id = export["entries"][0]["entry_id"].as_str().unwrap().to_string();
        ...
        fs::write(&signed_path, serde_json::to_string_pretty(&signed).unwrap() + "\n").unwrap();
        ...
        assert_eq!(stamped["entries"][1]["verification_status"], VERIFICATION_STATUS_VALID);
        let _ = fs::remove_dir_all(&root);
    }
```

Run: `cargo test -p xtask --lib verify_newswire_export` → all pass (GREEN), including the new
`signature_invalid` arm.

**2.7 — Commit.**

```
cargo fmt --all && cargo clippy -p xtask --all-features -- -D warnings
git add crates/xtask/src/verify_newswire_export.rs crates/xtask/src/main.rs
git commit -m "feat(xtask): verify-newswire-export re-verifies signatures + stamps status

Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>"
```
(`main.rs` is in this commit for the `mod verify_newswire_export;` line only — dispatch
registration is Task 3.)
...
**3.3 — Register the commands.** In `main.rs` (the two `mod` lines are ALREADY present from
Tasks 1.9 and 2.3 — do NOT re-add them):
...
# Coverage gate — CANONICAL command (source of truth = .coverage-thresholds.json):
#   CI/primary gate uses llvm-cov (tarpaulin's ptrace engine HANGS on this workspace in CI):
cargo llvm-cov --workspace --all-features --fail-under-lines 95   # thresholds.llvm.lines
#   Full local composite (tarpaulin lines 97 + llvm lines/functions/regions/branches + JS):
scripts/web/coverage.sh
```
Do NOT use bare `cargo tarpaulin -p xtask` — wrong scope and wrong tool for the enforced gate.
