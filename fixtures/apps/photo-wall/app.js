...
function clearError() { if (!ready) return; error.textContent = ""; error.hidden = true; const count = rows.filter(validPhoto).length; status.textContent = count ? `${count} photos` : "No photos yet"; }
...
async function ensureSeeded() {
  ...
  const seeds = [
    ["photos/1-courtyard", { caption: "Tables coming together in the courtyard", data_url: seedDataURL(SEED_SVGS.courtyard), created_at: 1, author_id: DEMO_IDS.alex }],
    ["photos/2-tool-library", { caption: "The tool library is open", data_url: seedDataURL(SEED_SVGS.tools), created_at: 2, author_id: DEMO_IDS.sam }],
    ["photos/3-street-feast", { caption: "A long table for the street feast", data_url: seedDataURL(SEED_SVGS.feast), created_at: 3, author_id: DEMO_IDS.jo }],
  ];
...
function paint() {
  ...
  captionInput.disabled = sharing; fileInput.disabled = sharing;
  ...
  if (ready && error.hidden) status.textContent = valid.length ? `${valid.length} photos` : "No photos yet";
  resolveProfiles(valid.map((row) => row.value.author_id));
}

captionInput.addEventListener("input", paint);
fileInput.addEventListener("change", async () => {
  ...
  catch (failure) { if (revision !== selectionRevision) return; preparedDataURL = ""; showError(failure && failure.message === "too large" ? "That photo is still too large — choose a smaller one." : "That image couldn't be prepared. Choose a JPEG, PNG, WebP, GIF, or SVG."); }
