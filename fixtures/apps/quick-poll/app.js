...
function paint() {
  replaceButton.disabled = !ready; postButton.disabled = !ready || posting || !formValid(); const proposal = currentProposal();
  ...
  const relevant = voteRows.filter(validVote).filter((row) => row.key.startsWith(`votes/${proposal.id}/`) && row.value.choice < proposal.options.length); const counts = proposal.options.map(() => 0); let mine = -1; const mineKey = me ? voteKey(proposal.id) : "";
  relevant.forEach((row) => { counts[row.value.choice] += 1; if (mineKey && row.key === mineKey) mine = row.value.choice; }); const total = counts.reduce((sum, count) => sum + count, 0); const locked = Boolean(mineKey) && voteLocks.has(mineKey);
  optionsElement.replaceChildren(...proposal.options.map((label, index) => {
    const li = document.createElement("li"); const button = document.createElement("button"); button.type = "button"; button.className = "option" + (mine === index ? " mine" : ""); button.disabled = !ready || locked; button.setAttribute("aria-pressed", String(mine === index)); const voteText = counts[index] === 1 ? "1 vote" : `${counts[index]} votes`; const percent = total ? Math.round(counts[index] / total * 100) : 0; button.setAttribute("aria-label", `${label}, ${voteText}, ${percent} percent`);
