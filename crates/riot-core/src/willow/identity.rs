...
pub enum NamespaceKind {
    Communal,
    /// A personal space: an owned namespace whose ID is the root public key.
    /// Generated by [`super::owned::OwnedRoot`]. Distinct from `Communal` at the
    /// bit level (the namespace ID itself), so it can never be retrofitted.
    Owned,
}
...
pub(crate) fn os_fill(buf: &mut [u8]) -> Result<(), WillowError> {
