...
fn valid_none_floor_ticket_verifies_and_admits_an_iroh_dial() {
    let t = mint(&root(), NS, "none", 1, 10_000, DIGEST, None, None, None);
    assert!(t.verify());
...
fn a_tampered_signature_never_dials() {
    let mut t = mint(&root(), NS, "none", 1, 10_000, DIGEST, None, None, None);
    t.sig[0] ^= 0x01;
    assert!(!t.verify());
    assert_eq!(
...
fn a_flipped_require_floor_breaks_the_signature() {
    ...
    let mut t = mint(&root(), NS, "arti", 1, 10_000, DIGEST, None, None, None);
...
fn require_arti_without_arti_fails_closed_never_falls_back_to_iroh() {
    ...
    let t = mint(&root(), NS, "arti", 1, 10_000, DIGEST, None, None, None);
    assert!(t.verify());
    assert_eq!(
        admit_dial(&t, &caps(true, false), 1_000, 0),
...
fn an_expired_ticket_is_refused() {
    let t = mint(&root(), NS, "none", 1, 500, DIGEST, None, None, None);
    assert_eq!(
        admit_dial(&t, &caps(true, false), 1_000, 0),
...
fn an_epoch_below_the_durable_floor_is_a_rollback() {
    ...
    let t = mint(&root(), NS, "none", 2, 10_000, DIGEST, None, None, None);
    assert!(t.verify());
    assert_eq!(
...
fn an_unknown_floor_token_fails_closed_not_parsed_as_none() {
    let t = mint(&root(), NS, "nym", 1, 10_000, DIGEST, None, None, None);
    assert!(t.verify());
