...
//!   4. an entry timestamped past `expires_unix_seconds` is not authorised.
//!
...
use riot_core::willow::{
    decode_capability_canonic, system_snapshot, Entry, NamespaceId, Path, SubspaceId,
};
...
fn minted_editor_cap_authorises_articles_section_entry() {
    let fx = fixture();
    let now = system_snapshot().expect("clock").unix_seconds;
    ...
    assert_eq!(invite.expires_unix_seconds, now + 3_600);
    ...
    assert!(
        ...
        "editor cap must authorise an /articles/news entry"
    );
...
fn entry_past_expiry_is_not_authorised() {
    ...
    // Past the time box: not authorised.
    ...
    assert!(
        expired
            .into_authorised_entry(&cap, &fx.editor_secret)
            .is_err(),
        "an entry past expires_unix_seconds must not be authorised"
    );
}
