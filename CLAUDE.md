...
- **Coverage is a RATCHET FLOOR, not 100%** — `.coverage-thresholds.json` holds the real per-tool floors (measured 2026-07-22 on the expanded workspace: tarpaulin lines 94.52%, llvm branches 82.19%), set just below the measured values so the gate is green today and blocks regression. The old 100% was fiction (nothing read the file). Raise floors as coverage improves; never lower one without a committed justification. The Rust line floor is enforced in CI (`.github/workflows/ci.yml`).
